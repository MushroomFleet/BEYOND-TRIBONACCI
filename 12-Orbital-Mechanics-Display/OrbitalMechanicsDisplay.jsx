...
const generateStar = (seed) => {
  ...
  // Weighted spectral class selection based on stellar population
  let cumulative = 0;
  let spectralClass = 'M';
  ...
  const metallicity = hashToRange(propHash4, 0.001, 0.04); // Solar metallicity ~0.02
  const age = hashToRange(hashCoord(seed, 5, 0, 6n), 0.1, 10); // Billion years
  
  // Stellar radius from mass-luminosity relation
...
// ─────────────────────────────────────────────────────────────────────────────
// PLANET TYPES: Classification Based on Position and Properties
// ─────────────────────────────────────────────────────────────────────────────

const PLANET_TYPES = {
  MOLTEN: { name: 'Molten', icon: '🔥', color: '#ff4444', bgGradient: 'linear-gradient(135deg, #8B0000, #FF4500, #FFD700)' },
  ROCKY: { name: 'Rocky', icon: '🪨', color: '#8b7355', bgGradient: 'linear-gradient(135deg, #4a3728, #8b7355, #6d5a48)' },
  DESERT: { name: 'Desert', icon: '🏜️', color: '#d4a574', bgGradient: 'linear-gradient(135deg, #c2956e, #e8d4a8, #9c7f5c)' },
  OCEAN: { name: 'Ocean', icon: '🌊', color: '#4a90d9', bgGradient: 'linear-gradient(135deg, #1a4a7a, #4a90d9, #7ab8e8)' },
  EARTHLIKE: { name: 'Earth-like', icon: '🌍', color: '#4caf50', bgGradient: 'linear-gradient(135deg, #2d5a2d, #4caf50, #81c784)' },
  ICE: { name: 'Ice World', icon: '❄️', color: '#b8d4e8', bgGradient: 'linear-gradient(135deg, #8fb8d8, #d4e8f0, #f0f8ff)' },
  GAS_GIANT: { name: 'Gas Giant', icon: '🪐', color: '#d4a574', bgGradient: 'linear-gradient(135deg, #8b6914, #d4a574, #f4e4c4)' },
  ICE_GIANT: { name: 'Ice Giant', icon: '💠', color: '#64b5f6', bgGradient: 'linear-gradient(135deg, #1565c0, #64b5f6, #90caf9)' },
  DWARF: { name: 'Dwarf Planet', icon: '⚫', color: '#666', bgGradient: 'linear-gradient(135deg, #333, #666, #888)' },
};
...
const generatePlanet = (star, zones, planetIndex, starSeed) => {
  ...
  // Determine planet type based on position relative to zones
  let planetType;
  let mass, radius;
  ...
  if (orbitalRadius > zones.frostLine) {
    // Beyond frost line: giants more likely
    ...
    // Habitable zone: chance for Earth-like
    mass = 0.3 + 4 * massHash;
    radius = Math.pow(mass, 0.27) * (0.9 + 0.2 * sizeHash);
    ...
    // Between Venus zone and habitable, or between HZ and frost line
    mass = 0.1 + 3 * massHash;
    radius = Math.pow(mass, 0.27) * (0.9 + 0.2 * sizeHash);
  ...
  const escapeV = 11.2 * Math.sqrt(mass) / radius; // km/s, Earth reference
  const thermalV = 0.157 * Math.sqrt(temperature); // Hydrogen at temperature
  const hasAtmosphere = escapeV > 6 * thermalV;
  
  // Surface gravity
  const gravity = mass / (radius * radius);
  
  // Moons based on planet type
  let moonCount = 0;
...
const generatePlanetarySystem = (star, starSeed) => {
  ...
  // Planet count influenced by stellar mass and metallicity
...
export default function OrbitalMechanicsDisplay() {
  ...
  // Generate system from seed (pure functional, memoized)
  ...
  // Calculate view parameters for orbital diagram
  ...
  // Generate CSS keyframes for orbital animation
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          PARENT-CONSTRAINED PROCEDURAL GENERATION • SEED: {seed}
        </p>
      </div>
      ...
      <div style={{
        display: 'grid',
        gridTemplateColumns: 'minmax(300px, 1fr) minmax(500px, 2fr) minmax(300px, 1fr)',
        gap: '24px',
        maxWidth: '1600px',
        margin: '0 auto',
      }}>
          ...
          <div style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {[
                ...
                { label: 'Mass', value: `${star.mass} M☉`, color: '#d4a574' },
                { label: 'Luminosity', value: `${star.luminosity} L☉`, color: '#fff4ea' },
                { label: 'Radius', value: `${star.radius} R☉`, color: '#b8d4e8' },
                  ...
                  <div style={{
                    ...
                    <div>
                      <div style={{
                        ...
                        {planet.designation} • {planet.typeData.name}
                      </div>
                      <div style={{
                        fontSize: '0.65rem',
                        color: '#666',
                      }}>
                        {planet.orbitalRadius} AU • {planet.orbitalPeriod} yr orbit
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedPlanet === i && (
                    <div style={{
                      ...
                      {[
                        ...
                        { label: 'Mass', value: `${planet.mass} M⊕` },
                        { label: 'Radius', value: `${planet.radius} R⊕` },
      ...
      <div style={{
        ...
        <div style={{
          ...
          {[
            { title: 'Stellar Mass', derived: 'System Extent', formula: 'outer_limit = 40 × M^0.5' },
            { title: 'Stellar Luminosity', derived: 'Zone Positions', formula: 'frost_line = 2.7 × √L' },
            { title: 'Orbital Position', derived: 'Planet Type', formula: 'r > frost_line → giants' },
            { title: 'Zone + Position', derived: 'Temperature', formula: 'T = 278 × L^0.25 / √r' },
            { title: 'Mass + Radius', derived: 'Atmosphere', formula: 'v_esc > 6 × v_thermal' },
            { title: 'Metallicity', derived: 'Earth-like Chance', formula: 'P = 0.15 × (Z / 0.02)' },
        ...
        <p style={{
          ...
          produces identical results—position is seed, and the universe springs complete from coordinates alone.
        </p>
      </div>
    </div>
  );
